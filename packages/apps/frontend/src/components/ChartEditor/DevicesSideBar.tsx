...
interface DevicesSideListProps {
  devicesList: Device[];
  systemElementsList: OverlayElement[];
    ...
    >
      ...
      {activeTab === "elements" && (
        <div className="flex flex-col flex-1 min-h-0 gap-3">
          ...
          {/* ── System elements (Note, Zone, Cloud…) ── */}
          <div>
            ...
            <ul className="space-y-1.5">
              ...
              {/* System overlay elements (e.g. Cloud) */}
              {systemElementsList.map((se) => (
                <li
                  key={se.id}
                  draggable
                  onDragStart={(event) => {
                    event.dataTransfer.setData("application/reactflow-overlay-element", JSON.stringify({ overlayElementId: se.id }));
                    event.dataTransfer.effectAllowed = "move";
                  }}
                  className={[
                    "flex items-center gap-2 px-2 py-2 rounded-lg border border-transparent cursor-grab active:cursor-grabbing transition-colors select-none",
                  ...
                  title={`${se.name} — drag onto canvas`}
                >
                  <div
                  ...
                  <div className="min-w-0 flex-1">
                    <p className={["text-xs font-semibold leading-4", isDark ? "text-slate-200" : "text-slate-800"].join(" ")}>{se.name}</p>
                    <p className={["text-[10px]", isDark ? "text-slate-500" : "text-slate-400"].join(" ")}>Drag to name on canvas</p>
                  </div>
                </li>
              ))}
            </ul>
          ...
          <p className={["text-[10px] text-center", isDark ? "text-slate-600" : "text-slate-400"].join(" ")}>Drag an element onto the canvas</p>
        </div>
      )}
    </aside>
  );
